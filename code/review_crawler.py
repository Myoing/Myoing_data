    ...
    try:
        ...
        for result in results:
            try:
                ...
                result_name = (link_name_elem.get_attribute("title") or link_name_elem.text).strip()
                if result_name == store_name:
                    logging.info(f"일치하는 가게 발견: {result_name}")
                    detail_btn = result.find_element(By.CSS_SELECTOR, "a[data-id='moreview']")
        ...
        if not matched:
            logging.warning(f"검색 결과에서 '{store_name}'과(와) 일치하는 가게를 찾지 못함")
            return False
...
def scroll_and_collect_reviews(driver, store_name, target_count=50, scroll_wait=2.0):
    ...
    while len(reviews) < target_count and scroll_attempt < max_scroll_attempts:
        try:
            review_containers = driver.find_elements(By.CSS_SELECTOR, "div.inner_review")
            ...
            if len(review_containers) == last_count:
                scroll_attempt += 1
                logging.info(f"[{store_name}] 새로운 리뷰 로드 시도 {scroll_attempt}/{max_scroll_attempts}")
            else:
                scroll_attempt = 0
            ...
            for container in review_containers[len(reviews):]:
                try:
                    ...
                    try:
                        content_elem = container.find_element(By.CSS_SELECTOR, "div.review_detail div.wrap_review a.link_review p.desc_review")
                        try:
                            ...
                            pass
                        review_content = content_elem.text.strip().replace("더보기", "").replace("접기", "").replace("\n", " ").strip()
                    except Exception as ce:
                        review_content = ""
                    
                    # 평점 추출: <span class="figure_star on"> 개수로 계산
                    try:
                        star_elements = container.find_elements(By.CSS_SELECTOR, "div.review_detail div.info_grade span.starred_grade span.wrap_grade span.figure_star.on")
                    ...
                    # 작성일 추출
                    try:
                        date_elem = container.find_element(By.CSS_SELECTOR, "div.review_detail div.info_grade span.txt_date")
                    ...
                    try:
                        reviewer_elem = container.find_element(By.CSS_SELECTOR, "div.info_user > div.wrap_user > a.link_user > span.name_user")
                    ...
                    logging.info(f"[{store_name}] 리뷰 수집: {len(reviews)}/{target_count}")
    ...
    if len(reviews) < target_count:
        logging.warning(f"[{store_name}] 목표 리뷰 수({target_count}개)에 도달하지 못했습니다. (수집된 리뷰: {len(reviews)}개)")
    return reviews
...
def process_store_reviews(store_record):
    ...
    try:
        ...
        if not search_store_detail(driver, store_name):
            logging.warning(f"[{store_name}] 상세 페이지 진입 실패: 가게 검색 또는 상세 페이지 이동 중 오류")
            return pd.DataFrame(), False

        reviews = scroll_and_collect_reviews(driver, store_name, target_count=50, scroll_wait=2.0)
        ...
        df = pd.DataFrame(collected_reviews, columns=["store_name", "store_address", "user_name", "user_rating", "review_date", "review_content"])
        return df, True
    except Exception as e:
        err_str = str(e).lower()
        if "invalid session id" in err_str:
            logging.error(f"[{store_name}] 세션 오류 발생: 브라우저 세션이 만료되었거나 연결이 끊어짐")
...
def main():
    ...
    2. 수집된 리뷰에 가게 정보(name, address) 추가 후 data/6_reviews_about_4 폴더에 두 개의 CSV 파일로 저장  
       - 모든 리뷰 데이터 파일 (empty review_content 포함): kakao_map_reviews_test_all.csv
       - 리뷰 내용이 비어있지 않은 행만 필터링한 파일: kakao_map_reviews_test_filtered.csv  
       CSV 칼럼 순서는 store_name, store_address, user_name, user_rating, review_date, review_content 입니다.
       
    ※ 기존 전체 실행 코드는 주석 처리하고, 여기서는 전체 가게에 대해 실행합니다.
    """
    ...
    # 전체 데이터를 대상으로 실행 (테스트용 상위 10개가 아닌 전체)
    all_filtered_data = pd.read_csv(filtered_data_path)
    test_data = all_filtered_data  # 상위 10개 대신 전체 데이터 사용
    ...
    with ThreadPoolExecutor(max_workers=MAX_DRIVERS) as executor:
        future_to_store = {
            ...
            for _, store_row in test_data.iterrows()
        }
    ...
    if review_dfs:
        ...
        # 전체 데이터를 저장하는 파일
        output_path_all = os.path.join(output_dir, "kakao_map_reviews_test_all.csv")
        all_reviews_df.to_csv(output_path_all, index=False, encoding="utf-8-sig", quoting=csv.QUOTE_ALL)
        logging.info(f"전체 리뷰 저장 완료: {len(all_reviews_df)}개의 리뷰, 파일: {output_path_all}")
        ...
        output_path_filtered = os.path.join(output_dir, "kakao_map_reviews_test_filtered.csv")
        filtered_df.to_csv(output_path_filtered, index=False, encoding="utf-8-sig", quoting=csv.QUOTE_ALL)
        logging.info(f"리뷰 내용이 있는 리뷰 저장 완료: {len(filtered_df)}개의 리뷰, 파일: {output_path_filtered}")

        if failed_stores:
            failed_stores_path = os.path.join(output_dir, "failed_stores_test.txt")
            with open(failed_stores_path, "w", encoding="utf-8") as f:
                f.write("\n".join(failed_stores))
            logging.info(f"실패한 매장 목록 저장 완료: {len(failed_stores)}개, 파일: {failed_stores_path}")
    else:
